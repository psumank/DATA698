...
pedestrian_data <- read_csv("https://raw.githubusercontent.com/psumank/DATA698/master/Data/pedestrian-incidents-fulldata.csv")
pedestrian_hotspots <- read_csv("https://raw.githubusercontent.com/psumank/DATA698/master/Data/pedestrian-hotspots-fulldata.csv")
...
server <- function(input, output, session) {
  ...
  output$maptext <- renderUI({ 
    if (nrow(reactive_pedestrian_hotspots()) > 0) {
      HTML(paste("Clusted the total incident records ", formatC(nrow(reactive_pedincidents_data()), format="d", big.mark=",") , " into ", nrow(reactive_pedestrian_hotspots())  , " hotspots using DBSCAN:<br>"))
    }else {
  ...
  output$vzText <- renderUI({ 
    ...
    if (input$vztype == 'SLOWZONE') {
    ...
    } else if (input$vztype == 'LEADING_PEDESTRIAN_INTERVAL') {
      ...
      HTML("<p style=margin:0.8em;>The Safe Streets for Seniors program is an initiative aimed at increasing safety for older New Yorkers. Based on factors such as senior population density, injury crashes, and senior trip
           ...
           centers.</p>")
    } else if (input$vztype == 'SIP_INTERSECTIONS' || input$vztype == 'SIP_CORRIDORS') {
      ...
      HTML("<p style=margin:0.8em;>All corridors in each borough were ranked on a pedestrian KSI (killed and severely injured) permile basis. Corridors were selected from the top of this list until the cumulative number of KSI
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(reactive_pedestrian_hotspots()) %>% addTiles() %>%
      setView(lng = -74.06, lat = 40.73, zoom = 11)  %>%
        addMarkers(lat= ~LATITUDE,lng=~LONGITUDE, label = ~htmlEscape(paste(ON.STREET.NAME, BOROUGH)))
  }) 
...
sidebar <- dashboardSidebar(
  ...
  selectInput(inputId = "vztype", label = "Vision Zero Initiative", choices = c("ANY", "SLOWZONE", "SPEED_HUMP", "SIGNAL_TIMING", "BIKE_PRIORITY", "ENHANCED_CROSSING", "LEADING_PEDESTRIAN_INTERVAL", "LEFT_TURN_TRAFFIC_CALMING", "NEIGHBORHOOD_SLOW_ZONE", "SAFE_STREETS_FOR_SENIORS", "SIP_INTERSECTIONS", "SIP_CORRIDORS", "PRIORITY_CORRIDORS", "PRIORITY_INTERSECTIONS", "PRIORITY_ZONES"), selected = "ANY"),
  ...
  sliderInput(inputId="samples", label="Samples", min=10, max=100, value=50, step = 10)
