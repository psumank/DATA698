...
nyc.collision.csv  <- read.csv("Data/NYPD_Motor_Vehicle_Collisions.csv", header=TRUE, sep=",", stringsAsFactors = FALSE, na.strings=c("NA", ""))
nyc.trafficViolations.csv  <- read.csv("Data/Traffic_Tickets_Issued__NYC.csv", header=TRUE, sep=",", stringsAsFactors = FALSE, na.strings=c("NA", ""))

df.coll <- data.frame(nyc.collision.csv)

pedestrian <- df.coll[(df.coll$NUMBER.OF.CYCLIST.INJURED > 0) | (df.coll$NUMBER.OF.CYCLIST.KILLED > 0) | (df.coll$NUMBER.OF.PEDESTRIANS.INJURED > 0) | (df.coll$NUMBER.OF.PEDESTRIANS.KILLED > 0) ,]
